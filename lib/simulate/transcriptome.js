...
const GFF2 = require('../formats/mapping/gff.js').GFF2;
const _MAXSEED = 100000000;
const _MAXITER = 1000;

class SimulatedGene extends Gene {
  static _default_parameters () {
    ...
    let params = {
      ...
      exon_skipping_probability: 0.2,
      alternate_exon_5prime_probability: 0.1,
      alternate_exon_5prime_range: [10,20],
      alternate_exon_3prime_probability: 0.1,
      alternate_exon_3prime_range: [10,20],
      alternate_transcript_5prime_probability: 0.1,
      alternate_transcript_5prime_range: [10,20],
      alternate_transcript_3prime_probability: 0.1,
      alternate_transcript_3prime_range: [10,20],
      intron_retention_probability: 0.05
    };
    return params;
  }
  constructor (mapping,options) {
    ...
    if (! options) options = {};
    this._direction = mapping.direction;
    this.params = SimulatedGene._default_parameters();
    ...
    // Now we have all the transcripts we are going to make
    for (let tx of candidate_txs) this.add_transcript(tx);
    // Now lets give all the exons unique IDs
    let exons = this.unique_exons;
    let ids = exons.entries.map(x=>rs.uuid4().toString());
    // Now Traveerse transcripts and set all exon ids
    for (let t of this.transcripts) {
      for (let e of t.exons) {
        let match_index = Array.from(new Range(0,exons.entries.length)).filter(x=>exons.entries[x].equals(e))[0];
        let id = ids[match_index];
        e.id = id;
      }
    }
  }
  ...
  _create_transcripts (mapping, rs, num_transcripts) {
    ...
    // num_transcripts.  May be called multiple times in attempts to make them!
    ...
    for (let i = 0; i < num_transcripts; i++) {
      ...
      if (exons.length === 0) continue; // no exons remain, this cant work
...
class SimulatedChromosome {
  ...
  constructor (options) {
    if (! options) options = {};
    this.params = SimulatedChromosome._default_parameters();
    if (options.params) fill_in_parameters(this.params,options.params);
  ...
  _get_mappings (chrname, length, rn, params) {
    ...
    while (true) {
      ...
      for (let i = 0; i < exon_count; i++) {
        ...
        pos += exon_length;
        let exon_end = pos;
        let exon = new Exon(chrname,exon_start,exon_end,direction);
        exons.push(exon);
...
class SimulatedTranscriptMapping extends Transcript {
  constructor (options) {
    if (! options) options = {};
    super(options);
