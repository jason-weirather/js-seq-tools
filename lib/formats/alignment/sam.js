...
class SAM extends GenericAlignment {
  ...
  get _map () {
    ...
    let refs = [new Bed(this.refName,this.pos-1,this.pos)];
    let soft_count = 0;
    let queries = [new Bed(this.read_name,0,1)];
    for (let i = 0; i < this.cigar.ops.length; i++) {
      ...
      if (/^[NDI]$/.test(op)) { // we will have a new exon
        ...
        refs[re].end = refs[re].start+1;
        qe = queries.length - 1;
        queries[qe].start = queries[qe].end;
        queries[qe].end = queries[qe].start+1;
      }
      if (op === 'N') {
        // Just an intron
        refs[re].start += num;
