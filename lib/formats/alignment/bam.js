...
class BAMOutputStream extends Transform {
  // output a bam stream
  constructor (options) {
    if (! options) {
      options = {};
...
class BAM {
  constructor (options) {
    ...
    if (options.data && options.header) {
      this.data = options.data; //begins with block_size
      this._header = options.header;
    } else if (options.sam_line) {
      this.sam_line = options.sam_line;
    }
    this.cache = false;
...
exports.BAMOutputStream = BAMOutputStream;
