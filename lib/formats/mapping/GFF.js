...
class GFF3 extends GFFGeneric {
  ...
  get_genes () {
    ...
    let output = [];
    let genes = {};
    let gene_ids = this._lines.filter(x=>x.type==='gene').map(x=>x.attributes.query({key:'ID'})[0].value);
    for (let gene_id of gene_ids) {
      genes[gene_id] = {};
    }
    let transcript_data = this._lines.filter(x=>x.type==='mRNA').map(get_data);
    let transcripts = {};
    for (let data of transcript_data) {
      if (! genes[data.parent]) {
        continue;
      }
      genes[data.parent][data.id] = 1;
      transcripts[data.id] = [];
    }
    ...
    for (let g of Object.keys(genes)) {
      ...
      for (let t of Object.keys(genes[g])) {
        ...
        let tx = new GFFTranscript({inmap:exons,name:t,gene_name:g,direction:strand});
        txs.push(tx);
      }
      //console.log(txs);
