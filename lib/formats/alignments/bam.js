...
var createBAMInputStream = function (options) {
  ...
  this._decompressor = bgzf.createBGZFDecompress();
  this._decompressed_data_processor = function (indata) {
    // the callback for decompressed bgzf data.  
    // Pileup data into bamreader
    that.bamreader.add(indata)
  }
  this._decompressor.on('data',this._decompressed_data_processor);
  //this._decompressor_pipe = this._decompressor.pipe();
}
...
createBAMInputStream.prototype._transform = function _transform(indata, encoding, callback) {
  var bamdata;
  this._decompressor.write(indata);
  console.log(this._decompressor.available());
  while (true) {
...
createBAMInputStream.prototype._flush = function (callback) {
  //var bamdata = this.bamreader.last();
  //this._decompressor;
  //console.log('no more data');
  //this._decompressor.write(null);
  //this._decompressor.end();
  //setTimeout(function() {}, 3000);
  //console.log(this._decompressor);
  //if (this._decompressor.cache.buffer.length > 0) {
  //  let remainder = this._decompressor.cache.buffer; 
  //  while (true) {
  //    let ndata = bgzf.gunzip_block(remainder);
  //    let indata = ndata.data;
  //    remainder = ndata.remainder;
  ...
  //}
  while (true) {
    let bamdata = this.bamreader.remove();
    if (! bamdata) break;
    if (bamdata) this.push(bamdata);
  }
  callback();
