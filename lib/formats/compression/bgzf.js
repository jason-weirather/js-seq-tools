...
var createBGZFCompress = function (options) {
  ...
  this.buffered = new Buffer(this.maxsize); // how much data can be stored
  this.datalen = 0; // how much data has been stored
  this.offset = 0;
  this.cache = new PipeFitter(this.maxsize);
...
createBGZFCompress.prototype._transform = function _transform(indata, encoding, callback) {
  ...
  while (true) {
    ...
    console.log(indata);
    if (! indata) { break; }
    raw_out = _gzip_block(indata,this.maxsize);
    //this.push(indata);
    this.push(raw_out);
  }
  callback();
